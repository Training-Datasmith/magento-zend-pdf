...
abstract class Zend_Pdf_Font
{
     ...
     */
    public static function font_with_name($name, $embedding_options = 0)
    {
        /* First check the cache. Don't duplicate font objects.
         */
        if (isset(Zend_Pdf_Font::$_font_names[$name])) {
            return Zend_Pdf_Font::$_font_names[$name];
        }
        /**
        ...
        switch ($name) {
            ...
            case Zend_Pdf_Font::FONT_HELVETICA_BOLD:
                ...
                $font = new Zend_pdf_resource_font_simple_standard_helvetica_Bold();
                break;
            case Zend_Pdf_Font::FONT_HELVETICA_OBLIQUE:
                #require_once 'Zend/Pdf/Resource/Font/Simple/Standard/HelveticaOblique.php';
                $font = new Zend_pdf_resource_font_simple_standard_helvetica_Oblique();
                break;
            case Zend_Pdf_Font::FONT_HELVETICA_BOLD_OBLIQUE:
                ...
                $font = new Zend_pdf_resource_font_simple_standard_helvetica_Bold_Oblique();
                break;
         ...
         */
        Zend_Pdf_Font::$_font_names[$name] = $font;
        return $font;
    }
    /**
     ...
     * @throws Zend_Pdf_Exception
     */
    public static function font_with_path($file_path, $embedding_options = 0)
    {
        /* First check the cache. Don't duplicate font objects.
         */
        $file_path_key = md5($file_path);
        if (isset(Zend_Pdf_Font::$_font_file_paths[$file_path_key])) {
            return Zend_Pdf_Font::$_font_file_paths[$file_path_key];
        }
        ...
        $data_source = new Zend_pdf_file_Parser_Data_Source_file($file_path);
        ...
        switch ($file_extension) {
            case 'ttf':
                $font = Zend_Pdf_Font::_extract_true_type_font($data_source, $embedding_options);
                break;
            default:
        ...
        if ($font !== null) {
             ...
             */
            $font_name = $font->get_font_name(Zend_Pdf_Font::NAME_POSTSCRIPT, '', '');
            Zend_Pdf_Font::$_font_names[$font_name] = $font;
            $file_path_key = md5($file_path);
            Zend_Pdf_Font::$_font_file_paths[$file_path_key] = $font;
            return $font;
        }
        /* The type of font could not be determined. Give up.
         */
        #require_once 'Zend/Pdf/Exception.php';
        throw new Zend_Pdf_Exception("Cannot determine font type: {$file_path}", Zend_Pdf_Exception::CANT_DETERMINE_FONT_TYPE);
    }
    /**** Internal Methods ****/
     ...
     * @throws Zend_Pdf_Exception
     */
    protected static function _extract_true_type_font($data_source, $embedding_options)
    {
        try {
            #require_once 'Zend/Pdf/FileParser/Font/OpenType/TrueType.php';
            $font_parser = new Zend_pdf_file_Parser_font_open_Type_true_Type($data_source);
            $font_parser->parse();
            if ($font_parser->is_adobe_latin_subset) {
                #require_once 'Zend/Pdf/Resource/Font/Simple/Parsed/TrueType.php';
                $font = new Zend_pdf_resource_font_simple_parsed_true_Type($font_parser, $embedding_options);
            } else {
                ...
                $cid_font = new Zend_pdf_resource_font_cid_Font_true_Type($font_parser, $embedding_options);
                $font = new Zend_Pdf_Resource_Font_Type0($cid_font);
            }
